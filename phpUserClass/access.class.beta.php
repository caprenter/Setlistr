...
class flexibleAccess{
   ...
   * var array
   * userID 	username 	password 	email 	activationHash 	active
   */
  ...
  */
  public function login($uname, $password, $remember = false, $loadUser = true) {
      ...
      $res = $this->query("SELECT activationHash FROM `{$this->dbTable}` 
      WHERE `{$this->tbFields['login']}` = '$uname' LIMIT 1",__LINE__);
      if ( @mysql_num_rows($res) == 0) {
        return false;
      } else {
        $salt = mysql_fetch_assoc($res);
        $salt = $salt["activationHash"];
        //echo $salt; //die;
        //echo $this->site_salt;
      }
    switch(strtolower($this->passMethod)){
      ...
      case 'hash_hmac':
        $password = hash_hmac("sha512",$salt.$password,$this->site_salt); break;
		  case 'sha1':
        $password = sha1($this->site_salt.$password.$salt); //echo "yes";
        break;
		  case 'md5' :
    ...
    //echo "SELECT * FROM `{$this->dbTable}` WHERE `{$this->tbFields['login']}` = '$uname' AND `{$this->tbFields['pass']}` = '$password' LIMIT 1";
		$res = $this->query("SELECT * FROM `{$this->dbTable}` WHERE `{$this->tbFields['login']}` = '$uname' AND `{$this->tbFields['pass']}` = '$password' LIMIT 1",__LINE__);
		if ( @mysql_num_rows($res) == 0)
			return false;
		if ( $loadUser ) {
  ...
  */
  ...
  public function insertUser($data){
    ...
    $data[$this->tbFields['salt']] = $this->randomPass();
    $data[$this->tbFields['pass']] = $this->encode_password($data[$this->tbFields['pass']],$data[$this->tbFields['salt']]);
    foreach ($data as $k => $v ) $data[$k] = "'".$this->escape($v)."'";
    $this->query("INSERT INTO `{$this->dbTable}` (`".implode('`, `', array_keys($data))."`) VALUES (".implode(", ", $data).")");
    //
    $this->userID = (int)mysql_insert_id($this->dbConn); //returns the user ID
    $_SESSION[$this->sessionVariable] = $this->userID; //set them in the session as logged in
  ...
  public function updateProperty($properties) {
    if(is_array($properties) && count($properties) > 0) {
      ...
      foreach($properties AS $k => $v) {
        if ($k == $this->tbFields['pass']) {
          $new_salt = $this->randomPass();
          $query .= "`activationHash` = '" . $new_salt . "',";
          $v = $this->encode_password($v,$new_salt);
        }
          
        //$v = ($k == $this->tbFields['pass']) ? $this->encode_password($v,$this->tbFields['salt']) : $v;
   ...
   * return string
   */
  public function randomPass($length=15, $chrs = '!£^*(?;,|1234567890qwertyuiopasdfghjklzxcvbnm'){
    $pwd ="";
    for($i = 0; $i < $length; $i++) {
  ...
  */
  private function query($sql, $line = 'Uknown') {
    if ( isset($this->DEVELOPMENT_MODE) && $this->DEVELOPMENT_MODE) echo '<b>Query to execute: </b>'.$sql.'<br /><b>Line: </b>'.$line.'<br />';
  ...
  */  
  ...
  private function encode_password($pass,$salt){
  	switch(strtolower($this->passMethod)){
    case 'hash_hmac':
      $password = hash_hmac("sha512",$salt.$pass,$this->site_salt); break;
    case 'sha1':
      $pass = sha1($this->site_salt.$pass.$salt);
     // echo $this->site_salt. "<br/>";
     // echo $pass. "<br/>";
     // echo $salt. "<br/>";
	  	return $pass;
	  case 'md5' :
